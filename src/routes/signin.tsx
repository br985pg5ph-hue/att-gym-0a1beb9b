...
export const Route = createFileRoute("/signin")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to Nuvo. Members, gym staff and platform admins all use the same door.",
      },
...
function SignInPage() {
  ...
  // One door for everyone: after the session exists we decide where it belongs.
  const routeOn = useCallback(
    async (userId: string) => {
      nav({ to: (await resolveEntry(userId)) as any });
    },
    [nav],
  );
        ...
        <div className="w-full sm:mx-auto sm:max-w-md">
          ...
          <div className="space-y-2">
            <button
              ...
              className="w-full rounded-pill border hairline bg-card py-3 text-sm font-medium"
            >
