...
ALTER TABLE public.children ADD CONSTRAINT children_gender_check CHECK (gender IS NULL OR gender IN ('male','female'));

-- Update handle_new_user to persist gender
...
BEGIN
  ...
  INSERT INTO public.profiles (id, name, phone, referred_by, gender)
  VALUES (
    NEW.id,
...
-- Enforce gender-based eligibility in consume_class_credit
...
BEGIN
  ...
  -- Gender-based restriction for female-only class types (adult bookings only)
  IF NOT is_staff_caller
     AND NEW.child_id IS NULL
     AND class_row.type::text IN ('women_only','yoga','gymnastics') THEN
    ...
    ELSIF member_gender <> 'female' THEN
      RAISE EXCEPTION 'This class is available to female members only';
    END IF;
  END IF;
